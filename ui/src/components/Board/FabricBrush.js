  ...
  fabric.util.object.extend(fabric.Object.prototype, {
    ...
    _updateEraserDimensions: function (dimensions, center) {
      ...
      if (eraser) {
        ...
        var newSize = fabric.util.object.extend(
          { width: size.x, height: size.y },
          dimensions
        );
        if (
          eraserSize.width === newSize.width &&
          eraserSize.height === newSize.height
        ) {
          return;
        }
        var offset = new fabric.Point(
          (eraserSize.width - newSize.width) / 2,
          (eraserSize.height - newSize.height) / 2
        );
        eraser.set(newSize);
    ...
    /** @lends fabric.EraserBrush.prototype */ {
      ...
      prepareCanvasOverlayForLayer: function (layer) {
        ...
        var renderOverlayOnTop =
          (image && !this._isErasable(image)) || hasOverlayColor;
        if (image && this._isErasable(image) === !erasablesOnLayer) {
          this.hideObject(image);
        }
